...
def trading_bot():
    ...
    while True:
        ...
        for symbol in crypto_symbols:
            ...
            if moving_avg:
                price_deviation = abs(current_price - moving_avg) / moving_avg * 100
                print(f"  - Price Deviation from Moving Average: {price_deviation:.2f}%")
                if price_deviation > 2:  # Only trade if price is within 2% of the moving average
                    print("  ⚠️ Price is too far from the moving average. Skipping trade.")
                    continue
            ...
            if price_change <= dynamic_buy_threshold and balances[quote_currency] > 0:
                ...
                print(f"  - Calculated Buy Amount: {buy_amount:.4f} {symbol}")
                if buy_amount >= min_order_sizes[symbol]["buy"]:
                    ...
                    print(f"🚫 Buy order too small: {buy_amount:.4f} {symbol} (minimum: {min_order_sizes[symbol]['buy']:.4f} {symbol})")

            # Check for sell condition
            elif price_change >= dynamic_sell_threshold and balances[symbol] > 0:
                sell_amount = (trade_percentage / 100) * balances[symbol]
                print(f"  - Calculated Sell Amount: {sell_amount:.4f} {symbol}")
                if sell_amount >= min_order_sizes[symbol]["sell"]:
                    ...
                    print(f"🚫 Sell order too small: {sell_amount:.4f} {symbol} (minimum: {min_order_sizes[symbol]['sell']:.4f} {symbol})")
